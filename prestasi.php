...
// DIUBAH: Query sekarang menghitung total data dari tabel `prestasi`
...
// DIUBAH: Query sekarang mengambil data dari tabel `prestasi`
                ...
                if ($prestasi_result && mysqli_num_rows($prestasi_result) > 0) {
                    while ($p = mysqli_fetch_assoc($prestasi_result)) {
                        ...
                        <div class="bg-gray-50 rounded-xl overflow-hidden flex flex-col transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                            <img src="upload/gambar_prestasi/<?= htmlspecialchars($p['gambar']) ?>" class="h-48 w-full object-cover" alt="Gambar: <?= htmlspecialchars($p['nama_prestasi']) ?>">
